...
const en = {
  ...
  patientsPickAria: (name: string) => `Add ${name} to the theatre list`,
  listTitle: "Theatre list",
  listPicked: (count: number) =>
    `${count} patient${count === 1 ? "" : "s"} picked, in the order you ticked them.`,
  listDay: "Day of surgery",
  listBuild: "Build the list",
  listBuilding: "Building…",
  listClear: "Clear the selection",
  listCopy: "Copy the list",
  listDownload: "Download as PDF",
...
const pt: Strings = {
  ...
  patientsPickAria: (name: string) => `Incluir ${name} na lista cirúrgica`,
  listTitle: "Lista cirúrgica",
  listPicked: (count: number) =>
    `${count} paciente${count === 1 ? "" : "s"} selecionado${count === 1 ? "" : "s"}, na ordem em que você marcou.`,
  listDay: "Dia da cirurgia",
  listBuild: "Gerar a lista",
  listBuilding: "Gerando…",
  listClear: "Limpar a seleção",
  listCopy: "Copiar a lista",
  listDownload: "Baixar em PDF",
