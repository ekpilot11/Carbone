...
interface BiometryPageProps {
  t: Strings;
  lang: Lang;
}

/**
 * The day-two work: photograph the exam printouts, check the values,
 * calculate, produce the record, and join it to the consultation the
 * patient had at their first visit.
 *
 * Lifted out of App.tsx unchanged when the app was split into pages. It is
 * the part of this app the clinic uses every day, so the move deliberately
 * edited none of its internals — only where it lives.
 */
export function BiometryPage({ t, lang }: BiometryPageProps) {
